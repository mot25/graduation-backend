...
import {TypeAction, TypeCollectionWithImages, TypeCollectionWithMainImage} from "../enums";
...
export class AddMainImageDto {
    ...
    @ApiProperty({type: "enum", required: true, description: "Коллекция", enum: Object.values(TypeCollectionWithMainImage), default: TypeCollectionWithMainImage.Events})
    @IsString()
    collection: string;
...
export class AddImagesResponse {
    @ApiProperty({type: Number, description: "Код ответа", default: 1})
    code: number;
